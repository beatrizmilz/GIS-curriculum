...
* [module9.gpkg](data/module9/module9.gpkg)
    * br_rj_capital
* [High Resolution Settlement Layer](data/module9/hrsl_rj_capital_populacao.tif)
* [SRTM Digital Elevation Model](data/module9/SRTM_DEM)
* [Global Land Cover Map 2015-2019 ](data/module9/Dynamic_Land_Cover_Map)
...
**Álgebra de mapa** representa um conjunto de operações primitivas<sup>[1]</sup> em um GIS que permite duas ou mais camadas raster de _dimensões semelhantes_ produzirem uma nova camada raster (mapa) usando várias operações, como adição, subtração, comparação etc.  
...
2. **Resolução temporal** está associada a imagens aéreas (imagens adquiridas por satélites, aviões, helicópteros, drones) e corresponde ao período entre 2 imagens consecutivas do mesmo ponto na Terra, tiradas nas mesmas condições (como tanto quanto possível), como a mesma aeronave, mesma altitude etc. Por exemplo, Landsat 8 tem uma resolução temporal de 16 dias, ou seja, cada ponto na Terra é revisitado pelo satélite Landsat 8 a cada 16 dias<sup>[2]</sup>.
...
![Espectro eletromagnético (crédito da foto [NASA Science](https://science.nasa.gov/ems/01_intro))](media/modulo9/fig94.png "Espectro eletromagnético (crédito da foto [NASA Science](https://science.nasa.gov/ems/01_intro)) ")
...
<table>
  ...
  <tr>
   ...
   <td>Pantanal. Herbáceo. Terras com mistura permanente de água e vegetação herbácea ou lenhosa. A vegetação pode estar presente em água salgada, salobra ou doce.
   </td>
  </tr>
  <tr>
   <td> 100
...
## Introdução ao trabalho com dados raster
...
Para calcular o número de pessoas com base nos dados raster HRSL que vivem até 200 metros de altitude na cidade do Rio de Janeiro, devemos ver quais pixels se enquadram em cada uma dessas categorias. Para fazer isso, usaremos a **Calculadora Raster**. Esta é uma funcionalidade que permite ao usuário realizar cálculos com base nos valores de pixel raster existentes. Os resultados são gravados em uma nova camada raster em um formato compatível com GDAL<sup>[5]</sup>.
...
## Trabalho com dados raster e vetoriais
...
![Exemplo de diferentes resoluções específicas para diferentes imagens de satélite - Landsat e SPOT - para a mesma área](media/modulo9/fig938.png "Exemplo de diferentes resoluções específicas para diferentes imagens de satélite - Landsat e SPOT - para a mesma área")
...
Voltando à nossa região de interesse - a cidade do Rio de Janeiro - podemos testar essas diferenças com os dados de que dispomos. Carregamos em nosso projeto QGIS as 5 camadas raster de LandCover por 5 anos: 2015, 2016, 2017, 2018 e 2019. Em seguida, carregaremos um mosaico de imagens do Sentinel-2<sup>[6]</sup>. Vamos carregar a camada WMS EOX Sentinel-2 sem nuvem, disponível [aqui](https://s2maps.eu/). Lembrando do módulo 2, para adicionar uma camada WMS, vá para **Camada > Adicionar camada > Adicionar camada WM/WMTS ..**
...
![Caixa para adicionar uma camada WMS ao QGIS](media/modulo9/fig939.png "Caixa para adicionar uma camada WMS ao QGIS")
...
![Camada sem nuvens Sentinel-2 para 2019 por EOX - 4326 para a cidade do Rio de Janeiro](media/modulo9/fig940.png "Camada sem nuvens Sentinel-2 para 2019 por EOX - 4326 para a cidade do Rio de Janeiro")
...
![Comparando 2 camadas raster usando o plugin da ferramenta MapSwipe](media/modulo9/fig941.png "Comparando 2 camadas raster usando o plugin da ferramenta MapSwipe")
...
![LandCover2019 obtido do PROBA-V (100m) no topo do mosaico Sentinel 2 (30m)](media/modulo9/fig942.png "LandCover2019 obtido do PROBA-V (100m) no topo do mosaico Sentinel 2 (30m)")
...
![HRSL adicionado no topo do Clipped_Reprojected_LandCover 2019](media/modulo9/fig943.png "HRSL adicionado no topo do Clipped_Reprojected_LandCover 2019.")
...
![Diferença na resolução espacial entre HRSL (30m - rosa) e Clipped_Reprojected_LandCover 2019 (100m - cores vermelho e magenta)](media/modulo9/fig944.png "Diferença na resolução espacial entre HRSL (30m - rosa) e Clipped_Reprojected_LandCover 2019 (100m - cores vermelho e magenta)")
...
![Reamostrando a camada de cobertura do solo](media/modulo9/fig945.png "Reamostrando a camada de cobertura do solo")
...
![Reamostragem do produto de cobertura do solo usando o método de reamostragem do Vizinho mais próximo](media/modulo9/fig946_a.png "Reamostragem do produto de cobertura do solo usando o método de reamostragem do Vizinho mais próximo")

Figura 9.46a - Reamostragem do produto de cobertura do solo usando o método de reamostragem do Vizinho mais próximo


![Reamostragem do produto de cobertura do solo usando o método de reamostragem de modo](media/modulo9/fig946_b.png "Reamostragem do produto de cobertura do solo usando o método de reamostragem de modo")
...
![Mostrar valores apenas para uma banda selecionada no histograma](media/modulo9/fig947.png "Mostrar valores apenas para uma banda selecionada no histograma")
...
![Histogramas de Clipped_Reprojected_LandCover](media/modulo9/fig948_a.png "Histogramas de Clipped_Reprojected_LandCover")

![LC2019_NearestNeighbour](media/modulo9/fig948_b.png "LC2019_NearestNeighbour")

![LC2019_Mode](media/modulo9/fig948_c.png "LC2019_Mode")
...
![Os 2 produtos raster: Land Cover 2019 e HRSL sobrepostos](media/modulo9/fig949.png "Os 2 produtos raster: Land Cover 2019 e HRSL sobrepostos")
...
![Poligonizando a camada raster LC2019_Mode (30m)](media/modulo9/fig950_a.png "Poligonizando a camada raster LC2019_Mode (30m)")

Figura 9.50a - Poligonizando a camada raster LC2019_Mode (30m)


![Resultado da poligonização do LC2019_Mode ( 30m) camada raster](media/modulo9/fig950_b.png "Resultado da poligonização do LC2019_Mode ( 30m) camada raster")
...
![LC2019_Mode polygonez](media/modulo9/fig951.png "LC2019_Mode polygonez")
...
![Incompatibilidade de pixels HRSL e o conjunto de dados vetoriais de cobertura do solo](media/modulo9/fig952.png "Incompatibilidade de pixels HRSL e o conjunto de dados vetoriais de cobertura do solo")
...
![Pixels raster para pontos](media/modulo9/fig953_a.png "Pixels raster para pontos")

Figura 9.53a - Pixels raster para pontos


![Execução do algoritmo de pixels raster para pontos](media/modulo9/fig953_b.png "Execução do algoritmo de pixels raster para pontos")
...
![Dados vetoriais do ponto de carregamento obtidos](media/modulo9/fig954.png "Dados vetoriais do ponto de carregamento obtidos")
...
![Criando um arquivo VRT com tiles raster específicos](media/modulo9/fig955.png "Criando um arquivo VRT com tiles raster específicos")
...
![Ladrilhos raster para o HRSL da cidade do Rio de Janeiro](media/modulo9/fig956.png "Ladrilhos raster para o HRSL da cidade do Rio de Janeiro")
...
![Rodando raster para pontos para todos os tiles raster](media/modulo9/fig957.png "Rodando raster para pontos para todos os tiles raster")
...
![Conjunto de dados vetoriais de pontos para a camada HRSL com valores de pixel armazenados na coluna VALUE](media/modulo9/fig958.png "Conjunto de dados vetoriais de pontos para a camada HRSL com valores de pixel armazenados na coluna VALUE")
...
![Verificando os valores dos dados de vetor de ponto vs. valores de pixel raster](media/modulo9/fig959.png "Verificando os valores dos dados de vetor de ponto vs. valores de pixel raster")
...
![Usando funções integradas com calculadora de campo](media/modulo9/fig960.png "Usando funções integradas com calculadora de campo")
...
![Usando funções integradas com calculadora de campo](media/modulo9/fig961.png "Usando funções integradas com calculadora de campo")

Figura 9.61 - Parâmetros do algoritmo de associação de atributos por localização (resumo)


![Saída dos atributos de Join por algoritmo de localização (resumo)](media/modulo9/fig962.png "Saída dos atributos de Join por algoritmo de localização (resumo)")
...
<sup>[1] Uma operação primitiva é um cálculo básico executado por um algoritmo.</sup>

<sup>[2]: A resolução temporal depende da latitude.</sup>

<sup>[3]: JAXA é a Agência Japonesa de Exploração Aeroespacial.</sup>

<sup>[4]: Para o propósito deste exercício, consideraremos o DSM como um modelo de elevação digital. Para obter mais detalhes sobre as diferenças, consulte a Divisão de conceitos seção. </sup>

<sup>[5]: A Geospatial Data Abstraction Library (GDAL) é uma biblioteca de software de computador para leitura e gravação de formatos de dados geoespaciais vetoriais e raster.</sup>

<sup>[6]: O Sentinel-2 é uma missão de observação da Terra do Programa Copernicus que adquire sistematicamente imagens ópticas em alta resolução espacial (10 ma 60 m) em águas terrestres e costeiras. Para mais detalhes, acesse [aqui](https://sentinel.esa.int/web/sentinel/missions/sentinel-2). </sup>
